...
[RequireComponent(typeof(RobotEnemyNavController))]
public class EnemyTurret : MonoBehaviour
{
    ...
    RobotEnemyNavController mRobotEnemyNavController;
    ...
    void Start()
    {
        ...
        mRobotEnemyNavController = GetComponent<RobotEnemyNavController>();

        mRobotEnemyNavController.onDetectedTarget += OnDetectedTarget;
        mRobotEnemyNavController.onLostTarget += OnLostTarget;
    ...
    private void CheckIfPlayerTryingToActivate(float arg0, GameObject bullet)
    {
        ...
        if (mRobotEnemyNavController.knownDetectedTarget)
            OnDetectedTarget();
    }
    ...
    {
        if (!mRobotEnemyNavController.isTargetInAttackRange) return;
        ...
        switch (aiState)
        {
            case AIState.Attack:
                ...
                Vector3 directionToTarget = (mRobotEnemyNavController.knownDetectedTarget.transform.position - turretAimPoint.position).normalized;
                ...
                if (mustShoot)
                {
                    ...
                    mRobotEnemyNavController.TryAtack(turretAimPoint.position + correctedDirectionToTarget);
                }
