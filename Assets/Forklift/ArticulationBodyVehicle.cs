...
public class ArticulationBodyVehicle : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) currentBreakForce = brakeForce;
        if (Input.GetKeyUp(KeyCode.Space)) currentBreakForce = brakeReleaseForce;
    ...
    private void UpdateVehicleAudio() => vehicleAudio.AdjustAudio(movementInput, rootArticulationBody.velocity.magnitude);
