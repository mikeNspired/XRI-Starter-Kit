...
public class DistanceGrabberLineBender : MonoBehaviour
{
    ...
    [SerializeField] ParticleSystem largeParticles;
    ...
    private float projectionDistanceMax;

    [SerializeField] private int particleMoveToCount;
    ...
    void UpdateSmallParticles()
    {
        ...
        //particleMoveToTargets[i] = particleSystem.transform.InverseTransformPoint(linerenderer.GetPosition(linerenderer.positionCount - (i + 2)));
    ...
    void UpdateLargeParticles()
    {
        ...
        float startLife = largeParticles.main.startLifetime.constant;

        for (int i = 0; i < totalParticles; i++)
        {
            //  if (particles[i].remainingLifetime / startLife < .75)   
